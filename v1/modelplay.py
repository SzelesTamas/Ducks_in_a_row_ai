...
class pygameBoard(Board):
    def __init__(self, state=None, startingPlayer: Literal[1, 2] = 1, screen: Any = None, linewidthPercent: float = 0.01, agent: NNAgent = None, agentInd = 1):
    ...
    def drawAgentScore(self):
        """Draws the predicted score for the current state"""
        
        value = self.agent.valueNetwork.getStateValue(Board.getStateForPlayer(self.state, self.agentInd))
        # draw the score
        text = self.font.render(str(value), False, (0, 0, 0), (255, 255, 255))
    ...
    def checkRelease(self, pos):
        ...
        Args:
            pos: mouse position
        """
        if(self.turn == self.agentInd):
            return
        if(self.selectedSquare is None):
            return
        square = self.getSquare(pos)
        if(square is not None):
            validMoves = Board.getValidMoves(self.state, self.turn)
    ...
    def agentMove(self):
        ...
        move, value = self.agent.getMove(self, self.agentInd)
        print(f"Move: {move}, Value: {value}")
        self.move(move[0], move[1], move[2], move[3], inPlace=True)

pygame.init()
pygame.font.init()
screen = pygame.display.set_mode((500, 500))

network = ValueNetwork("models/valueNetwork_zily_20230623-170517.h5")
agent = NNAgent(valueNetwork=network, randomness=0.0)
board = pygameBoard(screen=screen, agent=agent, agentInd=1)
running = True
mousePressed = False
while running:
