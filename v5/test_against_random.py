...
class GameManager:
    """This is a class for playing a number of games between a random agent and a QAgent."""
    ...
    def playGame(self, agent, agentInd):
        """Plays a game between a QAgent and a random agent and returns the winner.
        ...
        for i in range(self.maxGameLength):
            if onTurn == agentInd:
                move = agent.getMove(state, onTurn)
            else:
